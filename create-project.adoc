...
... *Choose a volume*: Select a volume on which to store the project. This volume is used as a software repository. You can only select volumes that are configured as an NFS share.
